...
import { Trash2, ShoppingBag, CreditCard, ChevronRight, Lock, MapPin, Truck, CheckCircle, Plus, Minus, AlertCircle, Info, Activity, Building, QrCode, Edit, X, ChevronLeft, Tag, Ban, FileText, Clock } from 'lucide-react';
import { DeliveryType, PaymentMethod, Order, OrderStatus, ProductCategory, Address, AppliedDiscount, DeliveryRegion } from '../types';
...
}> = ({ onSelect, selectedDate, checkAvailability, cart, region, getRegionInfo }) => {
    ...
    <div className="bg-white border rounded-2xl shadow-sm p-4 w-full">
      ...
      <div className="grid grid-cols-7 gap-1">
        {days.map((day, idx) => {
          ...
          if (availability.status === 'closed' || availability.status === 'full' || (region && !regionInfo.isOpen)) {
            bgColor = "bg-red-100 text-red-600";
            isBlocked = true;
            title = region && !regionInfo.isOpen ? 'V tento den se do tohoto regionu nerozváží.' : availability.reason;
          ...
          return (
            ...
            >
              <span>{day}</span>
              {regionInfo.isException && regionInfo.isOpen && (
...
export const Cart: React.FC = () => {
  ...
  // Check region availability for selected date
  ...
  const isRegionClosed = region && !regionInfo.isOpen;
  ...
  const handleSubmit = () => {
    if (!user || user.isBlocked || !date || (deliveryType === DeliveryType.DELIVERY && !region) || !availability?.allowed || !selectedBillingId || hasValidationErrors || isRegionClosed) return;
    ...
    const newOrder: Order = {
      ...
      deliveryType,
      deliveryDate: date,
      deliveryAddress: deliveryType === DeliveryType.PICKUP ? 'Osobní odběr' : `${selectedAddr?.name}\n${selectedAddr?.street}\n${selectedAddr?.city}\n${selectedAddr?.zip}\nTel: ${selectedAddr?.phone}`,
               ...
               {user && (
                 <>
                  ...
                  <div className={`bg-white p-6 rounded-2xl border shadow-sm ${user.isBlocked ? 'opacity-50 pointer-events-none' : ''}`}>
                    ...
                    {/* Modified Time Alert */}
                    {(region && regionInfo.isException && regionInfo.isOpen) && (
                      <div className="mt-4 p-3 bg-blue-50 text-blue-700 rounded-lg text-xs flex items-start">
                        ...
                        <span>Pro vybraný den ({date}) je upravený čas rozvozu.</span>
                      </div>
                    )}
                    ...
                    {isRegionClosed && (
                      <div className="mt-4 p-3 bg-red-50 text-red-700 rounded-lg text-xs flex items-start">
                        ...
                        <span>V tento den se do vybraného regionu nerozváží.</span>
                      </div>
                    )}
           ...
           <div className="bg-white p-8 rounded-3xl border shadow-xl sticky top-24">
              ...
              {step === 2 && (
                <div className="space-y-3">
                  <button 
                    disabled={!user || user.isBlocked || !date || (deliveryType === DeliveryType.DELIVERY && !region) || !availability?.allowed || !selectedBillingId || isRegionClosed} 
